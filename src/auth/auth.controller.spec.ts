...
describe('AuthController', () => {
  ...
  describe('loginUser', () => {
    it('should login a user', async () => {
      const token = 'Basic asdm';
      const reslut = {
        refreshToken: 'mocked.refresh.token',
        accessToken: 'mocked.access.token',
      };

      jest.spyOn(authService, 'login').mockResolvedValue(reslut);

      expect(authController.loginUser(token)).resolves.toEqual(reslut);
