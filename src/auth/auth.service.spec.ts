...
describe('AuthService', () => {
  ...
  describe('parseBearerTOken', () => {
    ...
    it('should throw a BadRequertException for invalid Bearer token formet', () => {
      const rawToken = 'a';
    ...
    it('should throw a BadRequestException if payload.type is not refresh but isRefreshToken parameter', () => {
  ...
  describe('register', () => {
    it('should register a new user', async ()=> {
      ...
      const reslut = await authService.register(rawToken);

      expect(authService.parseBasicToken).toHaveBeenCalledWith(rawToken);
      expect(userService.create).toHaveBeenCalledWith(user);
      expect(reslut).toEqual(user);
    });
  })
  ...
  describe('authenticate', () => {
    ...
    it('should  throw an error for not existing user', async() => {
      ...
      expect(authService.authenticate('test@test.com', 'passwod')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should thorw an error for incorrect password', async () => {
      ...
      await expect(authService.authenticate('test@test.com', 'password')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
